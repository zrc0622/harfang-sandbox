...
df.connect("10.243.58.131", 12345) # 连接环境
time.sleep(2) # 等待连接
df.disable_log() # 关闭日志？
render = False
...
# 抬升
# df.set_plane_pitch(plane_id, 0.1)
# df.set_plane_yaw(plane_id, 0.5)
df.set_plane_roll(plane_id, -0.5)
p = 0
while True:
# while p < 40:
	time.sleep(1/40)
	...
	action_list.append(action)
	state_list.append(state)
	a =plane_state["Euler_angles"][0]
	b=plane_state["pitch_attitude"]
	c =plane_state["Euler_angles"][2]
	d=plane_state["roll_attitude"]
	e =plane_state["Euler_angles"][1]
	f=plane_state["heading"]
	print(f"euler:{c,d}")
	# print(f"pitch:{b}")
